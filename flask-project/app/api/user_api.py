...
class UserView(FlaskView, MethodResource):
    ...
    @use_kwargs(
        UserSchema(only=("name", "email", "password"), partial=True), location="json"
    )
    ...
    def index(self, name=None, password=None, email=None):
        if name is None and password is None and email is None:
            if request.method in ["GET"]:
                user_model = (
                    UserModel.objects.filter(email=get_jwt_identity())
                    .only("email", "name")
                    .first()
                )
    ...
    def refresh(self):
        ...
        access_token = create_access_token(
            identity=user, expires_delta=datetime.timedelta(hours=1)
        )
    ...
    def login(self, email, password):
        ...
        return {
            "access_token": create_access_token(
                identity=email, expires_delta=datetime.timedelta(minutes=1)
            ),
            "refresh_token": create_refresh_token(
                identity=email, expires_delta=datetime.timedelta(weeks=2)
            ),
        }, 201
