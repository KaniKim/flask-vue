...
class ColumnView(FlaskView):
    ...
    def index(self):
        if request.method in ["GET"]:
            ...
            column_models = ColumnModel.objects.filter(author=user_id).only(
                "title", "tags"
            )
            ...
            return {
                "columns": [
                    {
                        "title": column_model.title,
                        "tags": [tag.name for tag in column_model.tags],
                    }
    ...
    def write_column(self, content, title, tags, name):
        if ColumnService(
            title=title, content=content, tags=tags, email=get_jwt_identity(), name=name
        ).save_column():
            return "SUCCESS", 201
        return "FAILED", 404
    ...
    def get_comment_column(self, column_id):
        ...
        comments = [
            CommentModel.objects.filter(id=comment.id).first()
            for comment in column.comments
        ]
            ...
            "comments": [
                {
                    ...
                    "author": UserModel.objects.filter(id=ObjectId(comment.author.id))
                    .first()
                    .name,
                    "next_comment": [
                        {
                            "content": CommentModel.objects.filter(
                                id=ObjectId(next_comment.id)
                            )
                            .first()
                            .content,
    ...
    def post_comment_column(self, column_id, content):
        ...
        if column.comments:
            ...
            column.comments = [
                CommentModel(content=content, author=user, next_comment=[]).save()
            ]
        column.save()
        return content, 201
...
class CommentView(FlaskView):
    ...
    def comment_next(self, comment_id, content):
        ...
        if comment:
            ...
            if comment.next_comment:
                ...
                comment.next_comment = [
                    CommentModel(content=content, author=user).save()
                ]
            comment.save()
            return content, 201
        return None, 404
...
class BoardView(FlaskView):
    ...
    def index(self):
        ...
        board_columns = [
            {
                "column": json.loads(
                    ColumnModel.objects.filter(id=column.id).first().to_json()
                ),
                "name": board.name,
            }
            for board in boards
    ...
    def get_column(self, board_name, column_id):
        ...
        return_value = {
            ...
            "tags": [
                TagModel.objects.filter(id=tag.id).first().name for tag in column.tags
            ],
            "title": column.title,
            "like": column.like,
        }
